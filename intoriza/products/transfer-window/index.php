<?php
$root = '../../';
$pageTitle = "Transfer Windows | Pass Boxes | Static & Dynamic | Pak Gusu";
$pageDescription = "Static and dynamic pass boxes (transfer windows) for material transfer in cleanrooms. Interlocking doors, UV sterilization, and HEPA filtration options.";
$pageKeywords = "transfer window, pass box, static pass box, dynamic pass box, cleanroom pass box, material transfer";
include($root . 'includes/head.php');
?>
<body class="footer-fixed">
    <div class="page-wraper">
        <?php include($root . 'includes/header.php'); ?>
        <!-- CONTENT START -->
        <!-- CONTENT START -->
        <div class="page-content">
        
            <!-- INNER PAGE BANNER -->
            <div class="wt-bnr-inr overlay-wraper bg-center"  style="background-image:url(<?php echo $root; ?>images/banner/1.jpg);">
            	<div class="overlay-main bg-black opacity-07"></div>
                <div class="container">
                    <div class="wt-bnr-inr-entry">
                    	<div class="banner-title-outer">
                        	<div class="banner-title-name">
                        		<h2 class="text-white">Transfer Windows / Pass Boxes</h2>
                            </div>
                        </div>
                        <!-- BREADCRUMB ROW -->                            
                        
                            <div>
                                <ul class="wt-breadcrumb breadcrumb-style-2">
                                    <li><a href="<?php echo $root; ?>index.php">Home</a></li>
                                    <li><a href="<?php echo $root; ?>products/index.php">Products</a></li>
                                    <li>Transfer Windows</li>
                                </ul>
                            </div>
                        
                        <!-- BREADCRUMB ROW END -->                        
                    </div>
                </div>
            </div>
            <!-- INNER PAGE BANNER END -->
             
            <!-- WELCOME SECTION START -->
            <div class="section-full p-t80 p-b50 bg-white">
                <div class="container">
                    <div class="section-content">
                    	<div class="row">
                        	<div class="col-lg-7 col-md-12 m-b30">
                            	<div class="welcome-carousel-outer m-b60">
                                	<div class="welcome-bg-block clearfix">
                                        <div class="welcome-carousel-1 ">
                                            <div class="owl-carousel home-carousel-1 owl-btn-bottom-left">
                                                <!-- COLUMNS 1 -->
                                                <div class="item">
                                                    <div class="ow-img">
                                                        <a href="javascript:void(0);"><img src="<?php echo $root; ?>images/welcome-slider/1.jpg" alt="Pak Gusu - Cleanroom Manufacturer Pakistan"></a>
                                                    </div>
                                                </div>
                                                <!-- COLUMNS 2 -->
                                                <div class="item">
                                                    <div class="ow-img">
                                                        <a href="javascript:void(0);"><img src="<?php echo $root; ?>images/welcome-slider/2.jpg" alt="Pak Gusu - Cleanroom Manufacturer Pakistan"></a>
                                                    </div>
                                                </div>
                                                <!-- COLUMNS 3 -->
                                                <div class="item">
                                                    <div class="owl-img">
                                                        <a href="javascript:void(0);"><img src="<?php echo $root; ?>images/welcome-slider/3.jpg" alt="Pak Gusu - Cleanroom Manufacturer Pakistan"></a>
                                                    </div>
                                                </div>
                                                <!-- COLUMNS 4 -->
                                                <div class="item">
                                                    <div class="ow-img">
                                                        <a href="javascript:void(0);"><img src="<?php echo $root; ?>images/welcome-slider/4.jpg" alt="Pak Gusu - Cleanroom Manufacturer Pakistan"></a>
                                                    </div>
                                                </div>
                                                <!-- COLUMNS 5 -->
                                                <div class="item">
                                                    <div class="ow-img">
                                                        <a href="javascript:void(0);"><img src="<?php echo $root; ?>images/welcome-slider/5.jpg" alt="Pak Gusu - Cleanroom Manufacturer Pakistan"></a>
                                                    </div>
                                                </div>
                                           </div>
                                        </div>                                 
                                   </div>
                               </div>
                            </div>
                    		<div class="col-lg-5 col-md-12 m-b30">
                                <!-- TITLE START -->
                                <div data-aos="fade-down" class="section-head">
                                	<div class="wt-separator-outer separator-left">
                                		<div class="wt-separator">
                                            <span class="site-text-primary text-uppercase sep-line-one ">Cleanroom Products</span>
                                        </div>
                                    </div>
                                    <h2>Transfer Windows / Pass Boxes</h2>
                                </div>
                                <!-- TITLE END -->                            
                                <p>Transfer windows (pass boxes) are used to move materials in and out of cleanrooms while minimising cross-contamination. Pak Gusu supplies pass box designs aligned with Gusu's dynamic and static pass box concepts.</p>
                                <p>Our transfer windows feature robust steel body with smooth internal corners, interlocking doors to prevent simultaneous opening, viewing windows to observe transfer, and optional UV lamps or other project-specific add-ons.</p>
                                 <a href="<?php echo $root; ?>products/index.php" class="site-button m-t15 m-b15">View All Products</a>
                            </div>                            
                        </div>
                        
                    </div>
                </div>
            </div>   
            <!-- WELCOME  SECTION END --> 

            <!-- WHAT WE DO SECTION START -->
            <div class="section-full p-t80 p-b50 bg-gray">
                <div class="container">
                    <!-- TITLE START -->
                    <div data-aos="fade-down" class="section-head text-center">
                        <div class="wt-separator-outer separator-center">
                            <div class="wt-separator">
                                <span class="site-text-primary text-uppercase sep-line-one ">Pass Box Types</span>
                            </div>
                        </div>
                        <h2>Types & Features</h2>
                    </div>
                    <!-- TITLE END -->
                    <div class="section-content">
                    	<div class="row">
                            <div class="col-lg-6 col-md-6 m-b30">
                            	<div class="hover-box-effect  v-icon-effect">
                                    <div data-aos="fade-up" class="wt-icon-box-wraper center p-lr30  p-b50 p-t50 bg-white">
                                        <div class="icon-lg site-text-primary m-b20">
                                            <span class="icon-cell site-text-primary"><i class="v-icon flaticon-sketch"></i></span>
                                        </div>
                                        <div class="icon-content text-black">
                                            <h4 class="wt-tilte m-b25">Types</h4>
                                            <ul class="list-angle-right text-left">
                                                <li><strong>Static pass box</strong> – For transfers between areas with similar cleanliness levels</li>
                                                <li><strong>Dynamic pass box</strong> – Integrated fan-filter unit (FFU) and HEPA filtration for transfers between areas of different cleanliness grades</li>
                                            </ul>
                                        </div>
                                    </div>
                                    
                                </div>
                            </div>                          
                            <div class="col-lg-6 col-md-6 m-b30">
                                <div class="hover-box-effect v-icon-effect">
                                    <div data-aos="fade-up" class="wt-icon-box-wraper center p-lr30  p-b50 p-t50  bg-white">
                                        <div class="icon-lg site-text-primary m-b20">
                                            <span class="icon-cell site-text-primary"><i class="v-icon flaticon-window"></i></span>
                                        </div>
                                        <div class="icon-content text-black">
                                            <h4 class="wt-tilte m-b25">Features</h4>
                                            <ul class="list-angle-right text-left">
                                                <li>Robust steel body with smooth internal corners</li>
                                                <li>Interlocking doors to prevent simultaneous opening</li>
                                                <li>Viewing windows to observe transfer</li>
                                                <li>Optional UV lamps or other project-specific add-ons</li>
                                            </ul>
                                        </div>
                                    </div>
                                    
                                </div>
                            </div>                          
                        </div>                                
                    </div>
                </div>  
            </div>   
            <!-- WHAT WE DO  SECTION END --> 

            <!-- PASS BOX SPECIFICATIONS SECTION START -->
            <div class="section-full p-t80 p-b50 bg-white">
                <div class="container">
                    <div class="section-head text-center">
                        <h2 class="text-uppercase">Technical Specifications</h2>
                        <div class="wt-separator-outer">
                            <div class="wt-separator style-square">
                                <span class="separator-left bg-primary"></span>
                                <span class="separator-right bg-primary"></span>
                            </div>
                        </div>
                    </div>
                    <div class="section-content">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                <th>Feature</th>
                                                <th>Specification</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td><strong>Material</strong></td>
                                                <td>Stainless Steel (SS304) / Powder Coated Steel</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Interlock</strong></td>
                                                <td>Mechanical / Electronic Interlock</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Filtration (Dynamic)</strong></td>
                                                <td>HEPA Filter (99.99% efficiency @ 0.3µm)</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Sterilization</strong></td>
                                                <td>UV Germicidal Lamp</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Power Supply</strong></td>
                                                <td>220V / 50Hz</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Accessories</strong></td>
                                                <td>Interphone, Differential Pressure Gauge (Dynamic)</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!-- PASS BOX SPECIFICATIONS SECTION END -->  
                        
                        
            
        </div>
        <!-- CONTENT END -->
        <!-- CONTENT END -->
        <?php include($root . 'includes/footer.php'); ?>
    </div>
    <?php include($root . 'includes/scripts.php'); ?>
</body>
</html>

